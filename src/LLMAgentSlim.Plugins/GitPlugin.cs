...
public class GitPlugin(string workspaceRoot) : WorkspacePluginBase(workspaceRoot)
{
	...
	[KernelFunction("git_branches")]
	[Description("Lists local and remote branches with their latest commit summary.")]
	public Task<string> GetBranches(string path = ".") =>
		ExecuteAsync(async () =>
		{
			var workingDirectory = ResolveWorkingDirectory(path);
			var result = await CommandRunner.RunProcessAsync(
	...
	[KernelFunction("git_show")]
	[Description("Shows details for a commit, tag, or other revision using 'git show --stat'.")]
	public Task<string> ShowRevision(string revision = "HEAD", string path = ".") =>
		ExecuteAsync(async () =>
		{
			...
			var result = await CommandRunner.RunProcessAsync(
				"git",
				["--no-pager", "show", "--stat", "--format=fuller", "-n", "1", revision],
				workingDirectory,
				30).ConfigureAwait(false);
