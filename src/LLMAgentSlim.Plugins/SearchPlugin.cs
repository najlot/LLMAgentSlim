...
public class SearchPlugin(string workspaceRoot) : WorkspacePluginBase(workspaceRoot)
{
	...
	[KernelFunction("search_regex")]
	[Description("Searches workspace files using a regular expression and returns matching paths, line numbers, and line snippets.")]
	public string SearchRegex(string pattern, string path = ".", int maxResults = 20, bool caseSensitive = false) =>
		Execute(() =>
		{
			if (string.IsNullOrWhiteSpace(pattern))
			{
	...
	[KernelFunction("search_filename")]
	[Description("Searches the workspace for files whose names contain the specified string, ignoring case.")]
	public string SearchFilename(string query, string path = ".") =>
		Execute(() =>
		{
			...
			var matches = EnumerateFiles(fullPath)
				.Select(filePath => PathResolver.ToRelativePath(filePath))
				.Where(relativePath => Path.GetFileName(relativePath).Contains(query, StringComparison.OrdinalIgnoreCase))
				.Take(200)
				.ToList();

			if (matches.Count == 0)
			{
				return $"No matching filenames found for '{query}'.";
			}
